...
- Settings modal that persists per-user data source preferences (stored in `.data/user-preferences.json`).
...
- Slack workspace with the Canvases feature (optional but recommended).
...
2. Paste the manifest in `manifest/product-slack-app-manifest.json` (update the URLs for your Render instance) or manually configure:
   - Scopes: `app_home:read`, `app_home:write`, `chat:write`, `files:write`, `users:read`, `users.profile:read` (+ `canvas:write` if available).
   - Enable App Home, Interactivity, and the `app_home_opened` event.
...
- **Prebuilt Canvas**: Sends one of the markdown templates from `src/prebuilt/canvases`. The template list appears in the first modal when this mode is chosen in Settings.

## Running the workflow
1. Open the app’s Home tab → pick a “View As” persona.
2. Hit **Executive Meeting Brief**:
   - Step 1: pick a template or prebuilt canvas.
...
4. Use the **Settings** button any time to switch among Mock / LLM / Prebuilt modes.
